...
class Builder(gymnasium.Env, gymnasium.utils.EzPickle):
    ...
    def _get_task(self) -> BaseTask:
        ...
        class_name = None
        if self.config and self.config.get('task_name'):
            task_name = self.config['task_name']
            if task_name.endswith(('0', '1', '2')):
                class_name = task_name[:-1] + 'Level' + task_name[-1]
    ...
    def reset(
        ...
        *,
        ...
        for agent in self.possible_agents:
            assert cost[agent]['cost_sum'] == 0, f'World has starting cost! {cost}'
    ...
    def step(self, action: dict) -> tuple[np.ndarray, float, float, bool, bool, dict]:
        ...
        if exception:
            ...
            costs = {agent: info['cost_exception'] for agent in self.possible_agents}
        else:
            # Reward processing
