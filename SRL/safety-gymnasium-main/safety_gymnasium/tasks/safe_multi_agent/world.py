...
class World:  # pylint: disable=too-many-instance-attributes
    ...
    def build(self):  # pylint: disable=too-many-locals, too-many-branches, too-many-statements
        ...
        # Move all agents to sampled starting positions
        num_agents = len(self.agent_xy)
        for idx in range(num_agents):
            body_name = 'agent' if idx == 0 else f'agent{idx}'
            body = None
            for candidate_body in worldbody['body']:
                if candidate_body.get('@name') == body_name:
                    body = candidate_body
                    break
            if body is None:
                continue

            body['@pos'] = convert(np.r_[self.agent_xy[idx], self._agent.z_height])
            body['@quat'] = convert(rot2quat(self.agent_rot[idx]))
        ...
        # Build and add a tracking camera for each agent body
        for idx in range(num_agents):
            body_name = 'agent' if idx == 0 else f'agent{idx}'
            body = None
            for candidate_body in worldbody['body']:
            ...
            camera_name = 'track' if idx == 0 else f'track{idx}'
            track_camera = xmltodict.parse(
                """<b>
                <camera name="{camera_name}" mode="track" pos="{xp} {yp} {zp}"
            ...
            if 'camera' in body:
                if isinstance(body['camera'], list):
                    body['camera'] = body['camera'] + [track_camera['b']['camera']]
                else:
                    body['camera'] = [body['camera'], track_camera['b']['camera']]
            else:
                body['camera'] = [track_camera['b']['camera']]
