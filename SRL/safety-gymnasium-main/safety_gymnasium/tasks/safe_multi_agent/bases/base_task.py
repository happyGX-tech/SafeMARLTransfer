...
class BaseTask(Underlying):  # pylint: disable=too-many-instance-attributes,too-many-public-methods
    ...
    def calculate_cost(self) -> dict:
        ...
        cost = {agent_name: {} for agent_name in self.agent.possible_agents}

        # Calculate constraint violations
        for obstacle in self._obstacles:
            obj_cost = obstacle.cal_cost()
            for agent_name in self.agent.possible_agents:
                if agent_name in obj_cost:
                    cost[agent_name].update(obj_cost[agent_name])
        ...
        if self.contact_other_cost:
            for contact in self.data.contact[: self.data.ncon]:
                ...
                hit_agent_indices = []
                for idx, body_info in enumerate(self.agent.body_info):
                    if any(n in body_info.geom_names for n in geom_names):
                        hit_agent_indices.append(idx)
                if len(hit_agent_indices) >= 2:
                    for idx in hit_agent_indices:
                        agent_name = self.agent.possible_agents[idx]
                        cost[agent_name]['cost_contact_other'] = self.contact_other_cost
    ...
    def _build_world_config(self, layout: dict) -> dict:  # pylint: disable=too-many-branches
        ...
        if self.agent.rot is None:
            world_config['agent_rot'] = self.random_generator.generate_rots(self.agent.nums)
        else:
    ...
    def build_goal_position(self) -> None:
        ...
        goal_names = getattr(self, 'goal_names', ['goal_red', 'goal_blue'])
        ...
        for idx, goal_name in enumerate(goal_names):
            if idx < len(self.goal_achieved) and self.goal_achieved[idx]:
                self.world_info.world_config_dict['geoms'][goal_name]['pos'][:2] = (
                    self.world_info.layout[goal_name]
                )
    ...
    def obs(self) -> dict | np.ndarray:
        ...
        for obstacle in self._obstacles:
            if obstacle.is_lidar_observed:
                for agent_idx in range(self.agent.nums):
                    suffix = '' if agent_idx == 0 else str(agent_idx)
                    obs[obstacle.name + '_' + f'lidar{suffix}'] = self._obs_lidar_by_index(
                        obstacle.pos,
                        obstacle.group,
                        agent_idx,
                    )
            if hasattr(obstacle, 'is_comp_observed') and obstacle.is_comp_observed:
                for agent_idx in range(self.agent.nums):
                    suffix = '' if agent_idx == 0 else str(agent_idx)
                    obs[obstacle.name + '_' + f'comp{suffix}'] = self._obs_compass(
                        obstacle.pos,
                        agent_idx=agent_idx,
                    )
    ...
    def _obs_lidar(self, positions: np.ndarray | list, group: int) -> np.ndarray:
        ...
        """
        return self._obs_lidar_by_index(positions, group, agent_idx=0)

    def _obs_lidar_by_index(
        self,
        ...
        if self.lidar_conf.type == 'pseudo':
            return self._obs_lidar_pseudo_by_index(positions, agent_idx)
    ...
    def _obs_lidar1(self, positions: np.ndarray | list, group: int) -> np.ndarray:
        ...
        See sub methods for implementation.
        """
        return self._obs_lidar_by_index(positions, group, agent_idx=1)
    ...
    def _obs_lidar_pseudo(self, positions: np.ndarray) -> np.ndarray:
        ...
        """
        return self._obs_lidar_pseudo_by_index(positions, agent_idx=0)

    def _obs_lidar_pseudo_by_index(self, positions: np.ndarray, agent_idx: int) -> np.ndarray:
        ...
        for pos in positions:
            ...
            z = complex(*self._ego_xy_by_index(pos, agent_idx))
            dist = np.abs(z)
    ...
    def _obs_lidar_pseudo1(self, positions: np.ndarray) -> np.ndarray:
        ...
        This encoding has some desirable properties:
            ...
            - constant size observation with variable numbers of objects
        """
        return self._obs_lidar_pseudo_by_index(positions, agent_idx=1)

    def _obs_compass(self, pos: np.ndarray, agent_idx: int = 0) -> np.ndarray:
        ...
        vec = pos - self.agent.pos(agent_idx)
        # Rotate into frame
        vec = np.matmul(vec, self.agent.mat(agent_idx))
        # Truncate
    ...
    def _ego_xy(self, pos: np.ndarray) -> np.ndarray:
        ...
        return self._ego_xy_by_index(pos, 0)

    def _ego_xy_by_index(self, pos: np.ndarray, agent_idx: int) -> np.ndarray:
        assert pos.shape == (2,), f'Bad pos {pos}'
        agent_3vec = self.agent.pos(agent_idx)
        agent_mat = self.agent.mat(agent_idx)
    ...
    def _ego_xy1(self, pos: np.ndarray) -> np.ndarray:
        """Return the egocentric XY vector to a position from the agent."""
        return self._ego_xy_by_index(pos, 1)
