...
class Underlying(abc.ABC):  # pylint: disable=too-many-instance-attributes
    ...
    def _build(self) -> None:
        ...
        if self.placements_conf.placements is None:
            ...
            self.random_generator.set_placements_info(
                ...
                goal_names=getattr(self, 'goal_names', None),
            )
    ...
    def _render_lidar(
        ...
        self._render_lidar_for_agent(0, poses, color, offset, group)

    def _render_lidar_for_agent(
        self,
        agent_idx: int,
        poses: np.ndarray,
        color: np.ndarray,
        ...
        """Render lidar markers for one agent."""
        agent_pos = self.agent.pos(agent_idx)
        agent_mat = self.agent.mat(agent_idx)
        lidar = self._obs_lidar_by_index(poses, group, agent_idx=agent_idx)
        for i in range(0, len(lidar), max(1, bin_stride)):
            sensor = lidar[i]
    ...
    def render(
        ...
        if self.render_conf.lidar_markers:
            lidar_obstacles = [obstacle for obstacle in self._obstacles if obstacle.is_lidar_observed]
            total_markers = self.agent.nums * len(lidar_obstacles) * self.lidar_conf.num_bins
            marker_budget = 700
            bin_stride = max(1, int(np.ceil(total_markers / marker_budget)))
            ...
            for obstacle in self._obstacles:
                if obstacle.is_lidar_observed:
                    for agent_idx in range(self.agent.nums):
                        self._render_lidar_for_agent(
                            agent_idx,
                            obstacle.pos,
                            obstacle.color,
                            offset + 0.02 * agent_idx,
                            obstacle.group,
        ...
        for idx, agent_name in enumerate(self.agent.possible_agents):
            if cost.get(agent_name, {}).get('cost_sum', 0) > 0:
                self._render_sphere(self.agent.pos(idx), 0.25, COLOR['red'], alpha=0.5)
