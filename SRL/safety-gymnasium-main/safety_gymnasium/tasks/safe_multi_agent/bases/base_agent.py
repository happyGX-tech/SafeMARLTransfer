...
from safety_gymnasium.tasks.safe_multi_agent.utils.multi_ant_xml import ensure_multi_ant_xml
...
class BaseAgent(abc.ABC):  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(  # pylint: disable=too-many-arguments
        ...
        """
        if name.lower() == 'ant':
            self.base = ensure_multi_ant_xml(num_agents)
        else:
            self.base = f'assets/xmls/multi_{name.lower()}.xml'
    ...
    def _sync_agent_topology(self) -> None:
        self.nums = self._infer_num_agents_from_model()
        self.possible_agents = [f'agent_{i}' for i in range(self.nums)]

    def _infer_num_agents_from_model(self) -> int:
        names = []
        for i in range(self.engine.model.nbody):
            body_name = self.engine.model.body(i).name
            if body_name == 'agent' or (body_name.startswith('agent') and body_name[5:].isdigit()):
                names.append(body_name)
        return max(len(names), 2)

    @staticmethod
    def _agent_body_name(index: int) -> str:
        return 'agent' if index == 0 else f'agent{index}'
    ...
    def _init_body_info(self) -> None:
        ...
        for i in range(self.nums):
            self.body_info[i].nq = int(self.engine.model.nq / self.nums)
            self.body_info[i].nv = int(self.engine.model.nv / self.nums)
            self.body_info[i].nu = int(self.engine.model.nu / self.nums)
            self.body_info[i].nbody = int(self.engine.model.nbody / self.nums)
    ...
    def _build_action_space(self) -> gymnasium.spaces.Box:
        ...
        low, high = bounds.T
        per_agent_dim = int(len(low) / self.nums)
        action_space = {}
        for idx, agent_name in enumerate(self.possible_agents):
            start = idx * per_agent_dim
            end = start + per_agent_dim
            action_space[agent_name] = spaces.Box(
                low=low[start:end],
                high=high[start:end],
                dtype=np.float64,
            )
    ...
    def mat(self, index: int) -> np.ndarray:
        body_name = self._agent_body_name(index)
        return self.engine.data.body(body_name).xmat.copy().reshape(3, -1)

    def pos(self, index: int) -> np.ndarray:
        body_name = self._agent_body_name(index)
        return self.engine.data.body(body_name).xpos.copy()
