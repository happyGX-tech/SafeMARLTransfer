...
class Vases(FreeGeom):  # pylint: disable=too-many-instance-attributes
    ...
    def cal_cost(self):  # pylint: disable=too-many-branches
        ...
        for idx, agent_name in enumerate(self.agent.possible_agents):
            agent_cost = cost[agent_name]

            agent_cost['cost_vases_contact'] = 0
            if self.contact_cost:
                for contact in self.engine.data.contact[: self.engine.data.ncon]:
                    ...
                    if any(n.startswith('vase') for n in geom_names) and any(
                        n in self.agent.body_info[idx].geom_names for n in geom_names
                    ):
                        self.last_contact[geom_names[0]] = idx
                        self.last_contact[geom_names[1]] = idx
                        agent_cost['cost_vases_contact'] += self.contact_cost

            if self.displace_cost:  # pylint: disable=no-member
                agent_cost['cost_vases_displace'] = 0
                for i in range(self.num):
                    ...
                    if dist > self.displace_threshold and self.last_contact[name] == idx:
                        agent_cost['cost_vases_displace'] += dist * self.displace_cost

            if self.velocity_cost:  # pylint: disable=no-member
                agent_cost['cost_vases_velocity'] = 0
                for i in range(self.num):
                    name = f'vase{i}'
                    vel = np.sqrt(
                        np.sum(np.square(get_body_xvelp(self.engine.model, self.engine.data, name))),
                    )
                    if vel >= self.velocity_threshold and self.last_contact[name] == idx:
                        agent_cost['cost_vases_velocity'] += vel * self.velocity_cost
