...
class Ant(BaseAgent):
    ...
    def reset(self):
        ...
        movements.
        """
        per_agent_nu = self.body_info[0].nu
        for agent_idx in range(self.nums):
            base_offset = agent_idx * per_agent_nu
            for joint_idx in range(per_agent_nu):
                noise = self.random_generator.uniform(low=-0.1, high=0.1)
                pos = noise if joint_idx % 2 == 0 else np.pi / 2 + noise
                actuator_id = base_offset + joint_idx
                joint_id = self.engine.model.actuator(actuator_id).trnid
                if joint_idx in (3, 5):
                    pos *= -1
