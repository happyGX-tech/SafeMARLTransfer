...
def make(
    ...
    **kwargs: Any,
    ...
    use_api_compatibility = apply_api_compatibility or (
        apply_api_compatibility is None and env_spec.apply_api_compatibility
    )
    if use_api_compatibility and EnvCompatibility is None:
        logger.warn(
            'apply_api_compatibility is requested but gymnasium.wrappers.compatibility.EnvCompatibility '
            'is unavailable in this gymnasium version; skipping compatibility wrapper.',
        )
        use_api_compatibility = False

    if use_api_compatibility:
