...
class FISOR(Agent):
    ...
    def create(
        ...
        if decentralized_actor:
            if obs_dim % num_agents != 0:
                raise ValueError(
                    f'Observation dim {obs_dim} is not divisible by num_agents={num_agents}. '
                    'Cannot build decentralized actor inputs.'
                )
            if action_dim % num_agents != 0:
                raise ValueError(
                    f'Action dim {action_dim} is not divisible by num_agents={num_agents}. '
                    'Cannot build decentralized actor outputs.'
                )
            local_obs_dim = obs_dim // num_agents
            local_act_dim = action_dim // num_agents
            actor_obs_dim = local_obs_dim
            actor_action_dim = local_act_dim
        else:
            local_obs_dim = obs_dim
            local_act_dim = action_dim
        ...
        if actor_architecture == 'mlp':
            base_model_cls = partial(MLP,
                                    hidden_dims=tuple(list(actor_hidden_dims) + [actor_action_dim]),
                                    activations=mish,
        ...
        actor_observations = observations[:, :actor_obs_dim]
        actor_actions = actions[:, :actor_action_dim]
        actor_params = actor_def.init(actor_key, actor_observations, actor_actions,
                                        time)['params']

        if actor_weight_decay is None or actor_weight_decay == 0.0:
            actor_optimiser = optax.adam(learning_rate=actor_lr)
        else:
            actor_optimiser = optax.adamw(
    ...
    def _reshape_actor_batch(self, observations: jnp.ndarray, actions: jnp.ndarray):
        if not self.decentralized_actor:
            return observations, actions

        batch_size = observations.shape[0]
        obs = observations.reshape((batch_size, self.num_agents, self.local_obs_dim))
        act = actions.reshape((batch_size, self.num_agents, self.local_act_dim))
        obs = obs.reshape((batch_size * self.num_agents, self.local_obs_dim))
        act = act.reshape((batch_size * self.num_agents, self.local_act_dim))
        return obs, act
    ...
    def update_actor(agent, batch: DatasetDict) -> Tuple[Agent, Dict[str, float]]:
        ...
        actor_observations, actor_actions = agent._reshape_actor_batch(
            batch['observations'],
            batch['actions'],
        )
        actor_batch_size = actor_actions.shape[0]

        if agent.sampling_method == 'dpm_solver-1':
            # continuous time SDE
            eps = 1e-3
            time = jax.random.uniform(key, (actor_batch_size, )) * (1. - eps) + eps
            key, rng = jax.random.split(rng, 2)
            noise_sample = jax.random.normal(key, (actor_batch_size, agent.actor_act_dim))
            alpha_t, sigma_t = vp_sde_schedule(time)
            time = jnp.expand_dims(time, axis=1)
            noisy_actions = alpha_t[:, None] * actor_actions + sigma_t[:, None] * noise_sample
        elif agent.sampling_method == 'ddpm':
            # discrete time SDE
            time = jax.random.randint(key, (actor_batch_size, ), 0, agent.T)
            key, rng = jax.random.split(rng, 2)
            noise_sample = jax.random.normal(key, (actor_batch_size, agent.actor_act_dim))
            ...
            noisy_actions = alpha_1 * actor_actions + alpha_2 * noise_sample
        else:
        ...
        def actor_loss_fn(
            ...
            actor_loss = (((eps_pred - noise_sample) ** 2).sum(axis = -1) * actor_weights).mean()
    ...
    def eval_actions(self, observations: jnp.ndarray):
        ...
        global_observations = jnp.expand_dims(observations, axis = 0).repeat(self.N, axis = 0)

        if self.decentralized_actor:
            per_agent_obs = observations.reshape((self.num_agents, self.local_obs_dim))
            local_action_candidates = []
            for agent_idx in range(self.num_agents):
                local_obs = jnp.expand_dims(per_agent_obs[agent_idx], axis=0).repeat(self.N, axis=0)
                if self.sampling_method == 'ddpm':
                    local_actions, rng = ddpm_sampler(
                    ...
                    local_actions, rng = dpm_solver_sampler_1st(
                        self.score_model.apply_fn,
                        score_params,
                        self.T,
                        rng,
                ...
                local_action_candidates.append(local_actions)

            actions = jnp.concatenate(local_action_candidates, axis=-1)
        else:
            if self.sampling_method == 'ddpm':
                actions, rng = ddpm_sampler(self.score_model.apply_fn, score_params, self.T, rng, self.act_dim, global_observations, self.alphas, self.alpha_hats, self.betas, self.ddpm_temperature, self.M, self.clip_sampler)
            elif self.sampling_method == 'dpm_solver-1':
                actions, rng = dpm_solver_sampler_1st(self.score_model.apply_fn, score_params, self.T, rng, self.act_dim, global_observations, self.alphas, self.alpha_hats, self.betas, self.ddpm_temperature, self.M, self.clip_sampler)
            else:
                raise ValueError(f'Invalid sampling method: {self.sampling_method}')
    ...
    def actor_loss_no_grad(agent, batch: DatasetDict):
        ...
        noise_sample = jax.random.normal(
            key, (actor_batch_size, agent.actor_act_dim))
